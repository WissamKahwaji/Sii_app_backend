...
export const getCoursesByCategory = async (req, res) => {
  try {
    ...
    const courses = await courseModel.find({ category: category }).populate([
      {
        path: "lessons",
        populate: [
          {
...
export const createCourse = async (req, res) => {
  try {
    ...
    const { name, price, duration, session, review, description, category } =
      req.body;

    const imgPath = req.files["img"][0].path;
    const urlImg =
      "https://www.siiappback.siidevelopment.com/" +
      imgPath.replace(/\\/g, "/");
...
export const createLesson = async (req, res) => {
  try {
    ...
    const { courseId } = req.params;
    const { name, duration, learningPoints } = req.body;
...
export const deleteLesson = async (req, res) => {
  try {
    ...
    if (lessonQuestionIds.length > 0) {
      await lessonQuestionsModel.deleteMany({
        _id: { $in: lessonQuestionIds },
      });
    }

    return res.status(200).json({
      message: "Lesson and related LessonQuestions deleted successfully",
    });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Something went wrong" });
